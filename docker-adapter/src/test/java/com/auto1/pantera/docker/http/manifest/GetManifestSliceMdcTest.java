...
/**
 * Regression tests: {@link GetManifestSlice} must propagate {@code user.name} MDC
 ...
 *
 * <p>Root cause: {@code AuthzSlice} sets {@code MDC.user.name} on the authentication
 * thread, but the Vert.x event loop thread that delivers the request body end-event
 * (via {@code body.asBytesFuture()}) is a different thread that does not inherit the
 * MDC context. Without this fix, {@code CacheManifests.requestOwner} was always null
 * for Bearer-token authenticated Docker pulls, causing {@code UNKNOWN} to be written
 * to the database.
 *
 * @since 1.20.13
 */
final class GetManifestSliceMdcTest {
    ...
    void setsMdcUserNameFromPanteraLoginHeaderBeforeCallingDockerLayer() {
        ...
        final GetManifestSlice slice = new GetManifestSlice(
            new MdcCapturingDocker(capturedMdc, manifest)
        );
        slice.response(
            new RequestLine(RqMethod.GET, "/v2/my-image/manifests/latest"),
    ...
    void setsMdcUserNameToUnknownWhenNoLoginHeader() {
        // When no artipie_login header is present (anonymous or pre-auth request),
        // Login.getValue() returns UNKNOWN, which must be propagated to MDC.
        final AtomicReference<String> capturedMdc = new AtomicReference<>("not-set");
                ...
                public Manifests manifests() {
                    return new Manifests() {
                        ...
                        public CompletableFuture<Optional<Manifest>> get(
                            ...
                            // Record MDC on whatever thread this runs on
