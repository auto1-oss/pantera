 ...
 * shape: 30 s seed, 60 min cap, trip on every 5xx / 401 / 407, trip
 * on every exception except local backpressure
 * ({@link RejectedExecutionException}). 429 is deliberately not in
 * the trip set — it is handled by the reactive rate-limit gate in
...
public record CircuitBreakerConfig(
    ...
    /**
     * Production defaults. 30 s seed, 60 min cap, Nexus-style trip
     * predicate (trip on 5xx / 401 / 407 and on every exception
     * except {@link RejectedExecutionException}).
     *
    ...
    /**
     * Default status-trip predicate. Trips on 5xx, 401, and 407.
     * Does NOT trip on 429 — that is owned by the rate-limit gate.
     *
     * @param status HTTP status code from the upstream response.
     * @return {@code true} iff the breaker should trip.
     */
    private static boolean defaultStatusTrip(final int status) {
        return status >= 500 || status == 401 || status == 407;
    }
}
