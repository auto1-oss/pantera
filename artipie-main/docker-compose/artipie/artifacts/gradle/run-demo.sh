#!/bin/bash
# Gradle Sample Demo Script for Artipie

set -e

echo "======================================"
echo "Gradle Sample - Artipie Demo"
echo "======================================"
echo ""

# Check if Artipie is running
echo "1. Checking Artipie server..."
if curl -s -f -u ayd:ayd http://localhost:8081/gradle_group > /dev/null 2>&1; then
    echo "   ✓ Artipie server is running"
else
    echo "   ✗ Artipie server is not accessible"
    echo "   Please start Artipie first: cd ../.. && docker-compose up -d"
    exit 1
fi
echo ""
...
# Publish to Artipie
echo "7. Publishing to Artipie gradle repository..."
./gradlew publish --console=plain
echo "   ✓ Published successfully"
echo ""

# Verify publication
echo "8. Verifying published artifacts in Artipie:"
