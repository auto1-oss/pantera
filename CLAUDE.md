...
- `proxy` — caching reverse proxy via `BaseCachedProxySlice`, a template-method base with a 7-step pipeline: negative-cache fast-fail → pre-process hook → cacheability → cache-first (offline-safe) → cooldown → deduplicated upstream fetch (single-flight) → store + digest. Adapters override `isCacheable`, `buildCooldownRequest`, `digestAlgorithms`, `buildArtifactEvent`, `postProcess`, `generateSidecars`.
...
**Cluster / HA.** `DbNodeRegistry` (PG heartbeats), `ClusterEventBus` (Valkey pub/sub, self-message filtering by instance UUID), `CacheInvalidationPubSub` (cross-instance Caffeine invalidation; its constructor blocks for the SUBSCRIBE ack — boot thread only, never event loop), Quartz JDBC mode (`PanteraScheduler` shared). Pure single-instance mode uses RAM Quartz and no Valkey.
...
- Unit tests (`*Test.java`, surefire) should not require Docker/network/DB — use `InMemoryStorage`. (Some legacy `*Test.java` use TestContainers; do not add new ones.) Service-dependent tests are `*IT.java`/`*ITCase.java` under `-Pitcase` (failsafe), backed by `test_images/`.
