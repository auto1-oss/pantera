...
**Supporting cache pieces.** `SingleFlight` (thundering-herd protection — concurrent same-key fills collapse to one upstream fetch + one cache write, held until the commit is durable), `NegativeCache` (L1 Caffeine + L2 Valkey), `ProxyCacheWriter` (stream-through tee with integrity verification; rollback after partial failure is fire-and-forget by design). Group serving is **stream-through**: a group relays the winning member's body straight to the client and never buffers it — dedup of concurrent same-key fills lives at the proxy member (`coalesceUpstream`), not the group, and `FileStorage` renames cache files atomically (`ATOMIC_MOVE`) so a concurrent reader never sees a half-written file.
...
**JWT auth.** RS256 (Auth0 java-jwt), one key pair, three token types (`access`, `refresh`, `api`) — the `type` claim is mandatory. Access tokens stateless; refresh/api tokens in `user_tokens` with revocation flags; access-token revocation broadcast over Valkey (entries under `pantera:revoked:user:{username}` / `pantera:revoked:jti:{jti}`, reloaded at boot, written through to `revocation_blocklist` when a DB is configured), 5 s table polling without Valkey. When adding a protected endpoint in `AsyncApiVerticle`, **reuse the shared `jwtAuthHandler` instance** — a new `JWTAuthHandler` skips the blocklist + JTI ownership checks.
...
**Correlation across async hops:** `EcsLoggingSlice` is the client entry point: it drops every client-sent internal header (`X-Pantera-Internal`, `pantera_login`, `X-Pantera-Ctx-*`) and stamps `X-Pantera-Ctx-Trace-Id` / `X-Pantera-Ctx-Client-Ip` on every inbound request; downstream slices restore MDC on worker threads via `RequestContextHeaders.bindToMdc(headers)` inside their async callbacks (the headers override stale pooled-thread MDC). `ArtifactEvent`/`ProxyArtifactEvent` auto-capture `traceId`/`clientIp` from MDC at construction; local publish paths build their event after async hops, so they bind the request's context explicitly with `ArtifactEvent.withRequestContext(headers)`. `DbConsumer` restores them before audit emission. One `trace.id` must connect the inbound request, outbound calls, pub/sub envelopes, Quartz jobs, and the audit record.
