#!/bin/bash
# Bump Artipie version in all locations using Maven Versions Plugin
...
echo "=== Artipie Version Bump (Multi-Module) ==="
echo ""
...
echo "  - Artipie-main:    $(grep -m 1 '<version>' artipie-main/pom.xml | sed 's/.*<version>\(.*\)<\/version>.*/\1/')"
echo ""
echo "Next steps:"
echo "  1. Review changes:  git diff pom.xml */pom.xml"
echo "  2. Test build:      mvn clean install -DskipTests"
echo "  3. Build Docker:    docker build -t artipie/artipie:$NEW_VERSION ."
echo "  4. Test locally:    cd artipie-main/docker-compose && docker-compose up -d"
echo "  5. Verify version:  docker logs artipie 2>&1 | jq '.service.version' | head -1"
