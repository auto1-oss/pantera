...
/**
 * Tests for {@link ComposerRootBaseLoader} — the WS6.3 TTL-cached,
 * single-flighted, serve-stale-on-failure loader for the Composer root
 * aggregation resolution surface ({@code /packages.json},
 * {@code /repo.json}).
 *
 ...
 * {@code GoMetadataBaseLoaderTest} (go-adapter) and
 * {@code PypiJsonBaseLoaderTest} (pypi-adapter), the reference tests for
 * this loader shape.
 *
 * @since 2.3.0
 */
final class ComposerRootBaseLoaderTest {

    @Test
    void warmCacheServesWithoutAnyUpstreamCall() throws Exception {
        final ScriptedUpstream upstream = new ScriptedUpstream();
        upstream.put("/packages.json", "{\"packages\":[]}");
        final Storage storage = new InMemoryStorage();
        final ComposerRootBaseLoader loader = newLoader(upstream, storage);

        final ComposerRootBaseLoader.Outcome first =
            loader.load("/packages.json").get(5, TimeUnit.SECONDS);
        MatcherAssert.assertThat(first.isAvailable(), new IsEqual<>(true));
        MatcherAssert.assertThat(
            new String(first.body(), StandardCharsets.UTF_8),
            new IsEqual<>("{\"packages\":[]}")
        );
        final Key key = new Key.From("root-api", "packages.json");
        awaitPersisted(storage, key);

        final ComposerRootBaseLoader.Outcome second =
            loader.load("/packages.json").get(5, TimeUnit.SECONDS);
    ...
    void servesStaleCopyWhenUpstreamFailsAfterTtlExpiry() throws Exception {
        ...
        MatcherAssert.assertThat(upstream.hits("/packages.json"), new IsEqual<>(1));

        // Force TTL expiry, then take upstream down.
        storage.stamp(key, Instant.now().minus(13, ChronoUnit.HOURS));
        upstream.fail(true);

        final ComposerRootBaseLoader.Outcome outcome =
            loader.load("/packages.json").get(5, TimeUnit.SECONDS);
        MatcherAssert.assertThat(
        ...
        MatcherAssert.assertThat(
            ...
            upstream.hits("/packages.json"), new IsEqual<>(2)
        );
    }
    ...
    void refetchesAfterTtlExpiryWhenUpstreamChanged() throws Exception {
        final ScriptedUpstream upstream = new ScriptedUpstream();
        upstream.put("/packages.json", "{\"packages\":[\"a\"]}");
        final FakeMetaStorage storage = new FakeMetaStorage(new InMemoryStorage());
        final ComposerRootBaseLoader loader = newLoader(upstream, storage);
        final Key key = new Key.From("root-api", "packages.json");
        ...
        storage.stamp(key, Instant.now().minus(13, ChronoUnit.HOURS));
        upstream.put("/packages.json", "{\"packages\":[\"a\",\"b\"]}");
    ...
    void doesNotRefetchWithinTtl() throws Exception {
        ...
        MatcherAssert.assertThat(
            ...
            upstream.hits("/packages.json"), new IsEqual<>(1)
        );
    }
    ...
    void singleFlightCollapsesConcurrentColdMissesToOneUpstreamCall() throws Exception {
        ...
        upstream.gate(gate);
        final ComposerRootBaseLoader loader = newLoader(upstream, new InMemoryStorage());

        final int concurrency = 8;
        final List<CompletableFuture<ComposerRootBaseLoader.Outcome>> futures =
            new ArrayList<>(concurrency);
        for (int idx = 0; idx < concurrency; idx++) {
            futures.add(loader.load("/packages.json"));
        }
        ...
        gate.complete(null);
        for (final CompletableFuture<ComposerRootBaseLoader.Outcome> future : futures) {
            final ComposerRootBaseLoader.Outcome outcome = future.get(5, TimeUnit.SECONDS);
    ...
    private static ComposerRootBaseLoader newLoader(final Slice upstream, final Storage storage) {
        final Cache cache = new FromStorageCache(storage);
        return new ComposerRootBaseLoader(upstream, cache, storage, "composer-test");
    }
     ...
     * asynchronously on completion — without this, a second {@link
     * ComposerRootBaseLoader#load(String)} call issued immediately after the
     * first can observe the still-cached (not yet invalidated) first result
     * instead of genuinely re-invoking the loader, which would make a
     * test's "TTL-expired, upstream now failing" setup silently ineffective.
     */
    private static void awaitSingleFlightSettled(final ComposerRootBaseLoader loader) throws Exception {
        final long deadlineNanos = System.nanoTime() + TimeUnit.SECONDS.toNanos(5);
