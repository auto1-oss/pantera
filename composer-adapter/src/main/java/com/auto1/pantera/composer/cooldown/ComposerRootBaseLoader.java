...
import com.auto1.pantera.composer.http.proxy.CacheTimeControl;
...
/**
 * Read-through, TTL-cached, single-flighted loader for the Composer root
 * aggregation resolution surface — {@code /packages.json} and
 * {@code /repo.json}.
 *
 * <p>Before this class existed, {@link ComposerRootPackagesHandler} fetched
 * the root document live from the raw upstream slice on <em>every</em>
 * request — no cache, no TTL, no fallback (WS6.3, the same gap closed for
 * Go's {@code @v/list}/{@code @latest} by {@code GoMetadataBaseLoader} and
 * for PyPI's JSON API by {@code PypiJsonBaseLoader}, both of which this
 * mirrors). A Packagist blip broke {@code composer require} root
 * resolution even for a package whose archive was already fully cached.
 * This loader closes it: the raw, unfiltered upstream body is cached
 * through {@link Cache} with a 12h {@link CacheTimeControl} TTL, concurrent
 * misses for the same document collapse onto one upstream call via
 ...
 *
 * <p>Callers ({@link ComposerRootPackagesHandler}) run per-request cooldown
 * filtering and URL rewriting over the returned base bytes, so this loader
 * deliberately caches the <em>unfiltered, unrewritten</em> document — a
 * cooldown lift/re-block is reflected on the very next request without
 * forcing a re-fetch.</p>
 *
 * @since 2.3.0
 */
public final class ComposerRootBaseLoader {
    ...
    /**
     * Storage namespace the root documents are cached under — distinct
     * from the per-package {@code <name>.json} cache keys sharing the same
     * repository storage.
     */
    private static final String NAMESPACE = "root-api";

    /**
     * Upstream slice — the raw remote (root aggregation has no per-package
     * name to route through the metadata-merge cache path).
     */
    private final Slice upstream;

    /**
     * Storage-backed cache the root documents are read through.
     */
     ...
     * cold root document collapse onto a single upstream call; followers
     * park then receive the leader's result.
     */
    ...
    /**
     * Resolve the root document at {@code path}, single-flighted per
     * storage key.
     *
     * @param path Request path, e.g. {@code /packages.json}
    ...
    private CompletableFuture<Outcome> fetchFromUpstream(
        ...
        final Remote remote = () -> this.upstream.response(line, Headers.EMPTY, Content.EMPTY)
            ...
            .exceptionally(err -> {
                EcsLogger.warn("com.auto1.pantera.composer.cooldown")
                    .message("Composer root base fetch failed")
                    .eventCategory("web")
                    .eventAction("root_base_fetch")
    ...
    private CompletableFuture<Outcome> staleFallback(
        ...
        return this.storage.exists(key).thenCompose(exists -> {
            ...
            return this.storage.value(key).thenCompose(Content::asBytesFuture).thenApply(bytes -> {
                EcsLogger.info("com.auto1.pantera.composer.cooldown")
                    .message("Serving stale Composer root base after upstream failure")
                    .eventCategory("web")
    ...
    /**
     * Namespace the request path under {@value #NAMESPACE} so the root
     * document cache never collides with the per-package cache keys
    ...
    /**
     * Outcome of resolving a Composer root document: either the document
     * bytes (fresh, cache-hit, or a stale fallback), or a status + body to
     * forward verbatim when nothing is available anywhere.
     *
     * @since 2.3.0
     */
