...
- **`pantera-backfill` records the real storage key for generic-file, helm and debian repositories** — `FileScanner` flattened the on-disk path into the dotted display name and discarded the original, so no amount of re-indexing could repair an affected row; the helm and debian scanners recorded nothing at all. All three now record it (helm from the index entry's `urls`, resolving the per-chart layout and falling back to the flat one; debian from the stanza's `Filename:` pool path), and each records a key only when it resolves to a file that is actually present — a fabricated path browses to nothing, which is the bug being fixed. This is what makes existing rows recoverable: re-run the backfill and their browse links resolve. The upsert is a `COALESCE`, so a re-run never clears a key another writer recorded.
  ([@aydasraf](https://github.com/aydasraf))
