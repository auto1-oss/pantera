...
- **Clicking "Browse" from a search result no longer lands in a non-existent directory** — the UI guessed that dots in a flat artifact name were directory separators, so a version like `1.0.0-SNAPSHOT` was split into fake nested folders; it now trusts the artifact's real stored path instead of guessing. ([@aydasraf](https://github.com/aydasraf))
