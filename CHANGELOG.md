...
- **Maven / Gradle cooldown rebuilt end-to-end.** A six-phase rebuild lands cooldown enforcement for Maven and Gradle, which had been effectively a no-op despite YAML configuration because per-version publish dates never reached the cooldown evaluator. (A) `DbConsumer` now writes `artifact_publish_dates` in the same transaction as `artifacts.release_date` (source `cache_write_event`); migration `V134__backfill_artifact_publish_dates.sql` populates the table from existing `artifacts.release_date` rows. `VertxMain` registers the `Last-Modified` extractor for every `*-proxy` repo-type variant (`maven-proxy`, `gradle-proxy`, `npm-proxy`, `pypi-proxy`, `composer-proxy`, `gem-proxy`, `go-proxy`), not just the bare names. (B) `CachedProxySlice.buildCooldownRequest` extracts the SNAPSHOT-timestamped version (`<base>-yyyyMMdd.HHmmss-N`) from the filename so each timestamped binary gets its own admission decision, not one shared one per `1.0-SNAPSHOT` directory. (C) New `MavenSnapshotMetadataParser` / `Filter` / `Rewriter` SPI triple — `<snapshotVersions>` is now first-class, `<updated>` is parsed as the inline release date, and `<snapshot><timestamp>` / `<buildNumber>` are rewritten to track the surviving newest entry on filter. (D) `<release>` is always recomputed when `blockedVersions` is non-empty, not only when `<latest>` was itself blocked — closes the Gradle `latest.release`-into-blocked-version regression that v2.2.0-RC carried. (E) Responses now emit a Pantera-computed weak ETag (sha256 of the filtered body) and a fresh `Last-Modified`; upstream `X-Checksum-*`, `CF-*`, `Age`, `X-Amz-*` no longer leak through the transform; inbound `If-None-Match` matching the computed ETag returns `304`. (F) New `PerInputFilteredMetadataCache` keys the filtered output by `(repoType, repoName, packageName, upstreamSha256)` plus a 1-hour cutoff bucket, so a new upstream version landing inside an in-memory entry is no longer hidden for 24 h. Maven-only; npm / PyPI / Composer keep their existing cache shape.
  ([@aydasraf](https://github.com/aydasraf))
- **`MetadataFilterService` now honours the full cooldown-override precedence chain.** Three sites (`enabledFor(repoType)` gate at the filter entry, `minimumAllowedAge()` cutoff in `computeFilteredMetadata`, and `minimumAllowedAge()` cutoff in `preWarmCooldownCache`) used global-only configuration. They ignored per-repo-type and per-repo-name overrides set via the admin UI / DB-backed `CooldownSettings`. Symptom: with global `minimum_allowed_age=30d` and `cooldown.repo_types.gradle-proxy.minimum_allowed_age=60d`, a version aged 40 days was silently allowed — the filter's pre-selection cutoff was 30d so the 40-day version never entered `versionsToEvaluate`, even though the gradle-proxy rule should have blocked it. Fix: added canonical `CooldownSettings.effectiveEnabled(repoType, repoName)` and `effectiveMinimumAllowedAge(repoType, repoName)` that walk the per-repo-name → per-repo-type → global ladder, mirroring `JdbcCooldownService.effectiveEnabled` / `effectiveDuration`. `MetadataFilterService` now uses the canonical methods; `JdbcCooldownService` refactored to delegate to them too (single source of truth, SNAPSHOT classifier ladder preserved on top). Affects every adapter routed through the shared filter (npm / PyPI / Composer / Gem / Maven / Gradle / Docker / Go).
  ([@aydasraf](https://github.com/aydasraf))
- **Removed the per-repo YAML `cache.cooldown.enabled` gate that silently disabled the admission-gate for every adapter that inherits from `BaseCachedProxySlice`.** The previous `BaseCachedProxySlice.evaluateCooldownAndFetch` / `evaluateCooldownOrProceed` gated on `this.config.cooldownEnabled() && cooldownService != null && cooldownInspector != null`. `ProxyCacheConfig.cooldownEnabled()` read the per-repo YAML key `cache.cooldown.enabled` and defaulted to `false` when absent — which it always was, for every repo YAML shipped. So operators who enabled cooldown via the admin UI / DB (the canonical control surface, `cooldown.enabled` and `cooldown.repo_types.<type>.enabled`) had the admission gate silently shut for every `.jar` / `.pom` / `.module` fetch through Maven, Gradle, PyPI, Composer, Docker, Go — every BaseCachedProxySlice-derived adapter. The `CooldownService` itself has the canonical `effectiveEnabled(request)` check that reads the live DB-backed `CooldownSettings`; the redundant pre-check was a footgun. Fix: removed both gate sites' use of `cooldownEnabled()` and deleted the method from `ProxyCacheConfig` (no remaining callers). The cooldown service is now the single source of truth — when disabled per the DB config it returns `CooldownResult.allowed()` immediately at near-zero cost; when enabled it actually fires.
  ([@aydasraf](https://github.com/aydasraf))
- **Maven / Gradle cooldown filter now consults the publish-date registry when inline release dates are empty.** Commit `dbdde1736` switched `MetadataFilterService.evaluateVersion` to read release dates exclusively from the in-memory `releaseDates` map populated by `MetadataParser.extractReleaseDates`. That fast-path optimisation was correct for npm / PyPI / Composer / Gem (inline dates exist) but inadvertently disabled Maven / Gradle: their artifact-level `maven-metadata.xml` carries no per-version timestamps, so the inline map was always empty, every version evaluated as "no release date — allowing", and the filter reported `"0 blocked"` regardless of how fresh anything was. Fix: when the inline date map is missing entries for versions in scope, `computeFilteredMetadata` now bulk-queries `PublishDateRegistries.instance().publishDate(...)` for the gaps, capped at `maxVersionsToEvaluate` (default 50) with a single 2-second whole-batch `allOf` timeout — never a per-version wall. Versions with inline dates short-circuit and never hit the registry, so npm / PyPI / etc. see zero overhead. Maven cold-path adds ~5–30 ms (50 parallel HikariCP queries) on first lookup; subsequent lookups hit Caffeine L1 in microseconds. The `gradle-proxy → maven-proxy` alias fallback in `DbPublishDateRegistry` (commit `0dc9c7387`) makes the historical rows reachable so gradle-proxy queries find Maven Central-sourced dates from previous fetches.
  ([@aydasraf](https://github.com/aydasraf))
- **`gradle-proxy` cooldown now actually fires.** `MavenProxyPackageProcessor` hard-coded `repo_type = "maven-proxy"` on every emitted `ArtifactEvent`, regardless of the actual repository type. Both `maven-proxy` and `gradle-proxy` repos route through the same `MavenProxy` slice (`RepositorySlices.java:739-740`), so every gradle cache-write landed in `artifact_publish_dates` under `repo_type='maven-proxy'`. The cooldown evaluator queried with `repo_type='gradle-proxy'` (correctly, from `cfg.type()`), missed, fell through to "no release date — allowing", and admitted every gradle artifact regardless of age. Forward fix: `ProxyArtifactEvent` carries a `repoType` field which `CachedProxySlice.buildArtifactEvent` populates from `this.repoType()`; `MavenProxyPackageProcessor` reads `event.repoType()` instead of the constant (preserving `"maven-proxy"` as the null-fallback for legacy paths). Belt-and-suspenders for historical rows: `DbPublishDateRegistry.publishDate(...)` adds a one-tier alias fallback (`gradle-proxy` → also try `maven-proxy`; `gradle` → also try `maven`) that fires only on primary lookup miss and writes the hit through to L1 under the original key. New `DbPublishDateRegistryAliasTest` covers both alias pairs, no-drift for unrelated repo_types, and L1 write-through. Other adapters (npm / pypi / composer / gem / go) only host a single proxy repo_type each — their hardcoded constants are correct, untouched.
  ([@aydasraf](https://github.com/aydasraf))
- **SNAPSHOT classifier cooldown knob — global + per-repo.** Operators can now configure a stricter (or laxer) cooldown for SNAPSHOT artifacts than for releases. YAML adds a `snapshots:` block under `cooldown:` (global) and under any entry of `cooldown.repo_names:` (per-repo). Both sub-knobs are optional (`enabled?`, `minimum_allowed_age?`) and inherit from the parent. Precedence at evaluation time: per-repo-name SNAPSHOT → per-repo-name → global SNAPSHOT → per-type → global default. `JdbcCooldownService.effectiveEnabled` / `effectiveDuration` apply the SNAPSHOT policy only when the request's version matches the SNAPSHOT timestamp pattern. `CooldownHandler` GET/PUT round-trips the new block; `CooldownSupport.loadDbCooldownSettings` rehydrates it on restart. The admin Settings UI grows a "SNAPSHOT cooldown" card on the global page; `RepoEditView` grows a per-repo Cooldown card with a master toggle and four override fields (enabled, age, snapshots.enabled, snapshots.age), permission-gated on `api_cooldown_permissions:write`. Recommended default for internal-mirror operators: stricter on SNAPSHOTs than releases, since SNAPSHOTs are CI-pushed development builds with less review and a mutable coordinate.
  ([@aydasraf](https://github.com/aydasraf))
- **npm / PyPI / Composer / Gem cooldown filter no longer does per-version upstream lookups.** The cooldown metadata filter was firing `inspector.releaseDate(name, version)` on every version inside the cooldown window — re-fetching the upstream packument over HTTP to read `time[version]`, a value already parsed out of the packument the proxy had just received. Each lookup hit the 1.7-second `orTimeout` in `JdbcCooldownService.checkNewArtifactAndCache`, timed out, and "allowed" by default. Net effect: ~1.7 s wall-clock penalty per cold-cache packument with zero blocking. Under `npm install` fan-out this multiplied to tens of seconds of upstream re-downloads against `registry.npmjs.org` and equivalents. The fix introduces `CooldownService.evaluateWithKnownDate(request, knownReleaseDate)` which skips the inspector entirely; `MetadataFilterService` now passes the in-memory `releaseDates` map straight through to the block decision. Verdaccio's `@verdaccio/package-filter` ships this pattern by design. The dedicated 4-thread `cooldown-eval` executor pool that fronted the old fan-out was removed (no I/O on the filter path now). The `Optional<CooldownInspector>` parameter on `CooldownMetadataService.filterMetadata(...)` was dropped from the SPI — call sites in `npm-adapter`, `maven-adapter`, and `MavenGroupSlice` updated. Expected impact: cold-cache packument latency drops from ~1.7 s to single-digit ms.
  ([@aydasraf](https://github.com/aydasraf))
- **Removed unused publish-date HTTP sources for npm, PyPI, Composer, and Gem.** `NpmRegistrySource`, `PyPiSource`, `PackagistSource`, and `RubyGemsSource` have been deleted. They existed only to satisfy the speculative `inspector.releaseDate(...)` lookups eliminated above; the publish dates for every one of these ecosystems are already inline in the packument / simple index / `p2.json` / gem metadata response the proxy fetches anyway. `GoProxySource`, `MavenHeadSource`, `MavenCentralSource`, `JFrogStorageApiSource`, and `ChainedPublishDateSource` are kept — Go and Maven manifests genuinely lack inline release dates. `VertxMain` no longer registers the four removed sources; `DbPublishDateRegistry` still works for the kept ones (the publishDateClient WebClient is unchanged).
  ([@aydasraf](https://github.com/aydasraf))
- **`CachedNpmProxySlice` no longer emits body-less 200 responses on the cacheable-tarball metadata-cache hit path.** The metadata short-circuit returned `ResponseBuilder.ok().headers(meta.get().headers()).build()` — headers (including `Content-Length`) without a body. Clients saw `200 OK` with a content-length pointing at a tarball that wasn't there. The cache-hit case is correctly served by the dedup → origin path that reads from `NpmProxy`'s storage tier; the metadata-only branch is gone.
  ([@aydasraf](https://github.com/aydasraf))
