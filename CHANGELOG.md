...
- **Workflow surface reduced to two.** All perf workflows (PR gate, baseline, cold-bench) and the nightly integration run were removed to keep public CI minimal and predictable: `ci.yml` (build + test backend/UI; external contributors' runs start only after maintainer approval) and a manually dispatched `release.yml`. The perf harness stays in the repo for local runs (`cd performance && make setup && make smoke`/`make cell`).
- **Releases are manually dispatched** (Actions tab or `gh workflow run release.yml`): the version is read from the root POM, the workflow tags `v<version>`, runs the full integration suite (`-Pitcase`), attaches artifacts + `SHA256SUMS`, publishes a multi-arch Docker image to GHCR (`ghcr.io/auto1-oss/pantera`, amd64+arm64), and attests build provenance for binaries and image.
- **Supply-chain hygiene**: GitHub-owned actions only, each pinned by commit SHA; default workflow token permissions read-only; `dependabot.yml` added (weekly, grouped, maven + npm + github-actions; docker-compose test fixtures excluded).
