...
- **`pantera-backfill` records the real storage key for generic-file repositories** — `FileScanner` flattened the on-disk path into the dotted display name and discarded the original, so no amount of re-indexing could repair an affected row. It now records the repo-relative path it already reads off disk, which is what makes existing rows recoverable: re-run the backfill against a file repository and its browse links resolve. The upsert is a `COALESCE`, so a re-run never clears a key another writer recorded.
  ([@aydasraf](https://github.com/aydasraf))
