...
- **An opt-in index-accelerated S3 cache eliminates the per-hit S3 HEAD.** The existing disk cache (`DiskCacheStorage`) pays 1-2 synchronous S3 HEADs on every cache hit for existence/validation. A new `cache.mode: index` (opt-in per storage, default remains unchanged) routes through `CachedBlobStorage`: an in-memory `StorageIndex`, hydrated on boot from a scan of the disk cache, answers `exists`/`metadata`/`list` entirely from memory, and a disk-served `value()` hit never contacts the blob store at all. Concurrent cold-miss requests for the same key are single-flighted into exactly one blob-store `GET`/`HEAD` instead of one per caller.
  ([@aydasraf](https://github.com/aydasraf))
- **`cache.mode: index` writes are acknowledged from local disk, not a synchronous S3 round trip.** `save()` now defaults to async durable write-back: bytes land on disk and are acked immediately, then a bounded pool of background threads uploads to S3 with retry/backoff; a crash before the upload confirms replays it on the next restart from the on-disk index. A saturated write-back queue rejects new writes immediately (before any disk I/O) rather than growing unbounded, and a proxy cache fill degrades gracefully instead of failing the client's already-served read. The prior synchronous behaviour remains available per-repository via `cache.write-through: true`. Write-back queue depth and oldest-pending-upload age are exposed as Prometheus gauges with a Grafana panel and a `PanteraWriteBackQueueNearCapacity` alert that warns before the queue starts rejecting writes; the blob-store tier and disk cache also emit per-op rate/latency/throttle and hit-rate/eviction metrics on the Cache & Storage dashboard.
  ([@aydasraf](https://github.com/aydasraf))
...
- **PyPI's legacy JSON API and Composer's repository root are TTL-cached with serve-stale.** `/pypi/<pkg>/json` and Composer's `packages.json`/`repo.json` root were fetched live on every request; they now go through a TTL cache with single-flight and serve-last-known-good on an upstream outage, so a blip no longer breaks `poetry`/`composer` resolution for already-cached packages.
  ([@aydasraf](https://github.com/aydasraf))
- **The PyPI `/simple/` cooldown-filtered index is cached instead of recomputed per request.** Serving a cooldown-filtered PEP 503/691 Simple Index re-parsed the upstream index and re-evaluated every version's cooldown on every request, even when nothing had changed. The filtered index is now materialised into the shared cooldown-metadata cache (single-flighted, with a TTL capped by the earliest block expiry), so a repeat request for a hot package is served without re-filtering; it self-busts on upstream content change and reuses every existing cooldown invalidation hook (block/unblock, upload, cross-node), so a blocked version never leaks and an aged-out version reappears promptly.
  ([@aydasraf](https://github.com/aydasraf))
...
- **Clustered deployments no longer drop proxy-repository index/audit records from the package-processors.** The per-node post-ingestion processors that index and audit a proxied artifact (Maven/Gradle, npm, PyPI, Go, Composer) were scheduled through the cluster-shared Quartz store, so a repeating firing could land on a different node than the one holding the artifact's queue and silently skip it — losing search-index rows and `artifact_publish`/`artifact_access` audit records. They now run on each node's own scheduler, the same node-local path the artifact-event drain uses.
  ([@aydasraf](https://github.com/aydasraf))
- **Docker manifest `GET`/`HEAD` honour the client's `Accept` header.** A manifest request now returns `406 Not Acceptable` when the client accepts none of the manifest's media type (`*/*` and `type/*` wildcards respected) instead of serving a manifest body the client cannot parse; an absent `Accept` header serves as before.
  ([@aydasraf](https://github.com/aydasraf))
- **Docker proxy caches manifests per negotiated `Accept`-variant.** A proxied manifest was fetched upstream with a fixed media-type superset and cached under a key that ignored the request's `Accept`, so for a multi-variant tag a client asking for `manifest.v2+json` could be served — or `406`'d against — a cached OCI image index. The proxy now forwards the client's `Accept` upstream and keys the cache by the negotiated variant, so each media type is fetched and cached independently and an offline read serves the matching variant rather than cross-serving. Digest-addressed pulls (one immutable representation) and the four-modern-type negotiation limit (no schema1 conversion) are unchanged.
  ([@aydasraf](https://github.com/aydasraf))
