...
/**
 * WS1.7 presigned-direct-download tests for {@link GoSlice}: only the {@code
 * @v/*.zip} module-byte route is redirect-eligible; the {@code @v/*.info},
 * {@code @v/*.mod} and {@code @v/list} metadata routes MUST keep streaming
 * even under a {@link DownloadMode#REDIRECT} policy with a presign-capable
 * backend. This is the adapter-level proof that metadata is never redirected.
 */
@Timeout(15)
final class GoSlicePresignTest {

    private static final String USER = "Alladin";
    private static final String PASS = "openSesame";
    private static final String PRESIGNED = "https://blobs.example.test/example.com/hello/@v/v1.0.0.zip?sig=abc";

    @Test
    void zipRedirectsButInfoAndListStream() {
        final PresigningStorage storage = new PresigningStorage(new InMemoryStorage());
        storage.save(new Key.From("example.com/hello/@v/v1.0.0.zip"), content("zip")).join();
        storage.save(new Key.From("example.com/hello/@v/v1.0.0.info"), content("{\"Version\":\"v1.0.0\"}")).join();
        storage.save(new Key.From("example.com/hello/@v/list"), content("v1.0.0\n")).join();
        ...
        final Response zip = get(slice, "example.com/hello/@v/v1.0.0.zip");
        MatcherAssert.assertThat(
            "the module .zip byte route must redirect (302) under REDIRECT policy",
            zip.status().code(), new IsEqual<>(302)
        );
        MatcherAssert.assertThat(
            "redirect must point at the presigned URL",
            new RqHeaders.Single(zip.headers(), Location.NAME).asString(),
            new IsEqual<>(PRESIGNED)
        );
        MatcherAssert.assertThat(
            "exactly one presign for the single .zip GET",
            storage.presignCalls.get(), new IsEqual<>(1)
        );

        MatcherAssert.assertThat(
            "the .info metadata route must stream (200), never redirect",
            get(slice, "example.com/hello/@v/v1.0.0.info").status().code(), new IsEqual<>(200)
        );
        MatcherAssert.assertThat(
            "the @v/list metadata route must stream (200), never redirect",
            get(slice, "example.com/hello/@v/list").status().code(), new IsEqual<>(200)
        );
        MatcherAssert.assertThat(
            "metadata routes must not have triggered any further presign attempts",
            storage.presignCalls.get(), new IsEqual<>(1)
        );
    }

    private static Response get(final GoSlice slice, final String path) {
