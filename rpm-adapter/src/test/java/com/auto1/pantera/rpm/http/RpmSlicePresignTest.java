...
/**
 * WS1.7 presigned-direct-download tests for {@link RpmSlice}: the shared
 * catch-all GET route serves BOTH {@code .rpm} package bytes and {@code
 * repodata/} metadata, so only the {@code .rpm} route may redirect. A {@code
 * repodata/} index GET MUST keep streaming even under a {@link
 * DownloadMode#REDIRECT} policy with a presign-capable backend -- the
 * adapter-level proof that metadata is never redirected.
 */
@Timeout(15)
final class RpmSlicePresignTest {

    private static final String USER = "Alladin";
    private static final String PASS = "openSesame";
    private static final String PRESIGNED = "https://blobs.example.test/rpm-repo/nginx-1.0.rpm?sig=abc";

    @Test
    void rpmRedirectsButRepodataStreams() {
        ...
        MatcherAssert.assertThat(
            "an .rpm package GET must redirect (302) under REDIRECT policy",
            rpm.status().code(), new IsEqual<>(302)
        );
        MatcherAssert.assertThat(
            "redirect must point at the presigned URL",
            new RqHeaders.Single(rpm.headers(), Location.NAME).asString(),
            new IsEqual<>(PRESIGNED)
        );
        MatcherAssert.assertThat(
            "exactly one presign for the single .rpm GET",
            storage.presignCalls.get(), new IsEqual<>(1)
        );

        MatcherAssert.assertThat(
            "a repodata/ index GET must stream (200), never redirect",
            get(slice, "/repodata/repomd.xml").status().code(), new IsEqual<>(200)
        );
        MatcherAssert.assertThat(
    ...
    private static Response get(final RpmSlice slice, final String path) {
