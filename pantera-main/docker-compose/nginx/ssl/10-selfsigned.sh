...
if [ ! -f "$DIR/nginx.key" ] || [ ! -f "$DIR/nginx.crt" ]; then
  # The official nginx:alpine image does not ship openssl; pull it in
  # (dev stack only — this runs once per fresh checkout).
  if ! command -v openssl >/dev/null 2>&1; then
    echo "[pantera] openssl missing in image; installing for dev cert generation"
