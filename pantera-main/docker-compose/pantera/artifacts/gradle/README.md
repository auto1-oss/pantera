# Gradle Sample Project for Pantera

This is a sample Gradle project demonstrating how to:
1. **Download dependencies** from Pantera `gradle_group` repository
2. **Publish artifacts** to Pantera `gradle` repository
...
1. **Pantera server running** on `http://localhost:8081`
...
### 5. Publish to Pantera
```bash
./gradlew publishMavenJavaPublicationToPanteraRepository
```
...
After publishing, you can verify the artifact in Pantera:
...
- Verify Pantera server is running
...
- Verify network connectivity to Pantera

### Issue: Authentication errors
- Update credentials in `build.gradle`
- Or set environment variables:
  ```bash
  export Pantera_USERNAME=ayd
  export Pantera_PASSWORD=ayd
  ```
  
  Then update `build.gradle`:
  ```gradle
  credentials {
      username = System.getenv('Pantera_USERNAME')
      password = System.getenv('Pantera_PASSWORD')
  }
  ```
...
4. Publish to your Pantera instance
