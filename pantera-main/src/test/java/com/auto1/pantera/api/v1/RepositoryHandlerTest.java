...
public final class RepositoryHandlerTest extends AsyncApiTestBase {
    ...
    /**
     * Task 21 — settings.prefetch must round-trip through the existing
     * generic JSONB upsert path. The repository handler stores the body
     * verbatim, so any nested field under {@code repo.settings.*} that
     * Task 20's {@link com.auto1.pantera.settings.repo.RepoConfig#prefetchEnabled()}
     * reads back must survive the PUT → GET cycle untouched.
     */
    @Test
    void putUpdatesPrefetchFlagAndRoundTrips(final Vertx vertx, final VertxTestContext ctx)
        throws Exception {
        final WebClient client = WebClient.create(vertx);
        final JsonObject withPrefetchOff = new JsonObject()
            .put(
                "repo",
                new JsonObject()
                    .put("type", "maven-proxy")
                    .put("storage", new JsonObject().put("type", "fs").put("path", "/tmp"))
                    .put("settings", new JsonObject().put("prefetch", false))
            );
        final HttpResponse<Buffer> put = client
            .put(this.port(), AsyncApiTestBase.HOST, "/api/v1/repositories/maven_proxy")
            .bearerTokenAuthentication(AsyncApiTestBase.TEST_TOKEN)
            .sendJsonObject(withPrefetchOff)
            .toCompletionStage().toCompletableFuture()
            .get(AsyncApiTestBase.TEST_TIMEOUT, TimeUnit.SECONDS);
        Assertions.assertEquals(200, put.statusCode());
        final HttpResponse<Buffer> get = client
            .get(this.port(), AsyncApiTestBase.HOST, "/api/v1/repositories/maven_proxy")
            .bearerTokenAuthentication(AsyncApiTestBase.TEST_TOKEN)
            .send()
            .toCompletionStage().toCompletableFuture()
            .get(AsyncApiTestBase.TEST_TIMEOUT, TimeUnit.SECONDS);
        Assertions.assertEquals(200, get.statusCode());
        final JsonObject body = get.bodyAsJsonObject();
        // The handler returns the raw config (which may or may not be wrapped
        // in a "repo" envelope depending on the CRUD impl). Both shapes are
        // valid — peel a single envelope if present.
        final JsonObject repo = body.containsKey("repo") ? body.getJsonObject("repo") : body;
        Assertions.assertNotNull(
            repo.getJsonObject("settings"),
            "settings section must round-trip"
        );
        Assertions.assertEquals(
            Boolean.FALSE,
            repo.getJsonObject("settings").getBoolean("prefetch"),
            "settings.prefetch must round-trip exactly as written"
        );
        ctx.completeNow();
    }

    /**
     * Task 21 — flipping prefetch back on must round-trip too. Guards
     * against the upsert path silently dropping {@code true} booleans
     * (some serialisers omit "default" values).
     */
    @Test
    void putPrefetchTrueRoundTrips(final Vertx vertx, final VertxTestContext ctx)
        throws Exception {
        final WebClient client = WebClient.create(vertx);
        final JsonObject withPrefetchOn = new JsonObject()
            .put(
                "repo",
                new JsonObject()
                    .put("type", "maven-proxy")
                    .put("storage", new JsonObject().put("type", "fs").put("path", "/tmp"))
                    .put("settings", new JsonObject().put("prefetch", true))
            );
        final HttpResponse<Buffer> put = client
            .put(this.port(), AsyncApiTestBase.HOST, "/api/v1/repositories/maven_proxy_on")
            .bearerTokenAuthentication(AsyncApiTestBase.TEST_TOKEN)
            .sendJsonObject(withPrefetchOn)
            .toCompletionStage().toCompletableFuture()
            .get(AsyncApiTestBase.TEST_TIMEOUT, TimeUnit.SECONDS);
        Assertions.assertEquals(200, put.statusCode());
        final HttpResponse<Buffer> get = client
            .get(this.port(), AsyncApiTestBase.HOST, "/api/v1/repositories/maven_proxy_on")
            .bearerTokenAuthentication(AsyncApiTestBase.TEST_TOKEN)
            .send()
            .toCompletionStage().toCompletableFuture()
            .get(AsyncApiTestBase.TEST_TIMEOUT, TimeUnit.SECONDS);
