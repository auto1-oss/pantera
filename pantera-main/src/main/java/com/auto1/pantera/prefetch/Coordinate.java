 ...
 *
 * <p>Use the static factories {@link #maven(String, String, String)},
 * {@link #npm(String, String)}, or {@link #npmPackument(String)} to construct
 * instances; the {@link #path()} method renders the canonical upstream path
 * for the coordinate.</p>
 *
 ...
 * @param version          Artifact version. Empty string for npm packument
 *                         coordinates (which target {@code /<scope>/<name>}
 *                         metadata, not a versioned tarball).
 * @since 2.2.0
 */
public record Coordinate(
    ...
    public enum Ecosystem {
        ...
        MAVEN,
        /**
         * npm tarball (scoped or unscoped) — targets
         * {@code <scope>/<name>/-/<name>-<version>.tgz}.
         */
        NPM,
        /**
         * npm packument (metadata) — targets {@code <scope>/<name>}.
         * Phase 13: speculative packument prefetch warms the dep-tree
         * critical path ahead of npm install's serial walk.
         */
        NPM_PACKUMENT
    }
    ...
    /**
     * Build an npm packument (metadata) coordinate. Targets
     * {@code <scope>/<name>} on the upstream registry — i.e. the JSON
     * metadata document, not a tarball. Used by Phase 13 speculative
     * packument prefetch to warm the dep-tree critical path ahead of
     * npm install's serial walk.
     *
     * <p>Scoped names (starting with {@code @}) are split into scope and
     * bare name; unscoped names use an empty scope. Version is the empty
     * string — packument URLs are version-less.</p>
     *
     * @param packageName npm package name, e.g. {@code "@types/node"} or {@code "react"}.
     * @return npm packument coordinate.
     */
    public static Coordinate npmPackument(final String packageName) {
        final String scope;
        final String bare;
        if (packageName.startsWith("@") && packageName.contains("/")) {
            final int slash = packageName.indexOf('/');
        ...
        return new Coordinate(Ecosystem.NPM_PACKUMENT, scope, bare, "");
    }
     ...
     *   <li>npm packument scoped: {@code @scope/name}</li>
     *   <li>npm packument unscoped: {@code name}</li>
     * </ul>
     *
