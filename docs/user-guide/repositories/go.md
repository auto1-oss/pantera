...
**2. Upload** — `.info` and `.mod` first, `.zip` last (the zip adds the version to `@v/list`, which Pantera keeps in semver order; `@v/list` itself cannot be uploaded and answers `405`). Each upload answers `201`:
