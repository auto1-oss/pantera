...
(`.whl`/sdist), `conda` (`.tar.bz2`/`.conda`), `go` (`@v/*.zip`),
`maven`/`gradle` (`.jar`/`.pom`/`.war`/`.aar`/`.zip`/`.module`, including
classifier jars), `nuget` (`.nupkg`/`.snupkg`), `composer` (dist archives) and
`hexpm` (package tarballs)
repositories)
...
`maven-metadata.xml`, checksum/signature sidecars) is never redirected. Standard HTTP clients follow redirects transparently; the
