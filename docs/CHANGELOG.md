...
- **Per-repo anonymous-access controls.** A new `anonymous_read` / `anonymous_write` flag per repo decides whether unauthenticated requests get a `401` + `WWW-Authenticate: Basic realm="pantera"` or pass through to downstream auth. Defaults follow the Artifactory/Nexus convention — proxy repos default to read-on / write-off, hosted repos default to both off.
