...
Pantera v2.1.0 replaces HMAC-SHA256 (HS256) JWT signing with RSA-SHA256 (RS256). This is a **hard cutover** -- the old `meta.jwt.secret` field is no longer recognized and will cause a startup failure.

### Why a Hard Cutover?

The HS256 shared secret in the open-source repository defaulted to `"some secret"`, meaning any user familiar with the public codebase could forge arbitrary JWTs. Keeping HS256 alive during a transition period would maintain this vulnerability. All pre-upgrade tokens must therefore be invalidated.

### Migration Steps

**Step 1: Generate an RSA key pair (on the Pantera host or a secure machine).**
...
For HA deployments, copy both PEM files to all nodes (or mount from shared storage).

**Step 2: Update `pantera.yml`.**
...
And mount the key files in `docker-compose.yml`:

```yaml
services:
  pantera:
    volumes:
      - /path/to/jwt-private.pem:/etc/pantera/jwt-private.pem:ro
      - /path/to/jwt-public.pem:/etc/pantera/jwt-public.pem:ro
```

**Step 4: Notify users.**

All existing tokens (HS256-signed) are invalid after the upgrade. Users must:

1. Log in again via `POST /api/v1/auth/token` or the Management UI.
2. Re-generate API tokens for CI/CD pipelines: `POST /api/v1/auth/token/generate`.
3. Update stored tokens in Maven `settings.xml`, `.npmrc`, `pip.conf`, Docker credential stores, etc.

Send users the updated [Getting Started](../user-guide/getting-started.md) guide before the maintenance window.

**Step 5: Verify.**

After starting the new version, confirm RS256 is active:

```bash
# The alg header in the token should be "RS256", not "HS256"
curl -s -X POST http://localhost:8086/api/v1/auth/token \
  -H "Content-Type: application/json" \
  -d '{"name":"admin","pass":"changeme"}' | jq -r '.token' | \
