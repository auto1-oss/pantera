...
| `PANTERA_BOOTSTRAP_ADMIN_PASSWORD` | (none) | Initial password of the `admin` user created on the first database-backed start (only while no `admin` user or other `admin`-role holder exists). When unset, a random password is generated and written to `<pantera.home>/bootstrap-admin-password` (default `/var/pantera/bootstrap-admin-password`, mode `0600`), never to the log; if that file cannot be written the user is not created. |
