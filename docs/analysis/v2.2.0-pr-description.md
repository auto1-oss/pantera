...
- [x] **Phase 1** -- SOLID package restructure (`api/`, `cache/`, `metadata/`, `response/`, `config/`, `metrics/`, `impl/`)
- [x] **Phase 2** -- 5 performance hardenings: H1 (pre-warm release-date cache), H2 (parallel bounded evaluation, 4 threads), H3 (SWR on metadata cache), H4 (50K L1 capacity), H5 (inflight-map leak fix)
- [x] **Phase 3** -- Per-adapter metadata parser/filter/rewriter/detector for 7 adapters (235+ unit tests)
- [x] **Phase 4** -- Per-adapter 403 response factories with `CooldownResponseRegistry`
- [x] **Phase 5** -- Admin unblock flow hardened (sync invalidation, CooldownCache + FilteredMetadataCache both invalidated, Micrometer counters)
- [x] **Phase 6** -- `CooldownAdapterBundle<T>` record + `CooldownAdapterRegistry` populated at startup; all 7 adapters wired
- [x] **Phase 7** -- Integration tests (MetadataFilterServiceIntegrationTest, CooldownAdapterRegistryTest) + chaos test (100-concurrent stampede dedup)
- [x] **Phase 8** -- Documentation + changelog + final verification
