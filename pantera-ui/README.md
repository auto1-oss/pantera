...
Web management console for [Pantera](https://github.com/auto1-oss/pantera) — a binary artifact repository manager.
...
- Node.js 22+
- npm 10+
- A running Pantera instance with the `/api/v1/*` endpoints (AsyncApiVerticle on port 8086)
...
# Run (assumes Pantera is reachable at http://pantera:8086 from within Docker network)
docker run -d -p 8090:80 --name pantera-ui pantera-ui
```

### With Docker Compose

The UI is included in the main Pantera `docker-compose.yaml`. It builds from this directory and is available at **http://localhost:8090**.
...
| `APP_TITLE` | Application title in header and login page | `Pantera` |
...
pantera-ui:
  environment:
    ...
    - APP_TITLE=${UI_APP_TITLE:-Pantera}
...
- **`/api/`** -- Proxied to `http://pantera:8086/api/` (Pantera REST API)
